...
clear
echo "========================================================================="
echo "Welcome to AutoStack (OpenStack Essex Multi Node )"
echo ""
echo ""
echo "AutoStack v0.0.3 by badb0y"
echo ""
...
#get server ip:
getntpip(){
read -p "Ntp Server IP:" ntpip
if [[ $ntpip = "" ]];then
    getntpip
fi
}
getcontrolip(){
read -p "Control Server IP:" controlip
if [[ $controlip = "" ]];then
    getcontrolip
fi
}
getcomputeip(){
read -p "Compute Server(compute local ip)IP:" computeip
if [[ $computeip = "" ]];then
    getcomputeip
fi
}
getmetadataip(){
read -p "Metadata IP:" metadataip
if [[ $metadataip = "" ]];then
    getmetadataip
fi
}
get_fixed_range(){
read -p "Fixed_range (192.168.17.0/24):" fixed_range
if [[ $fixed_range = "" ]];then
    get_fixed_range
fi
}
...
ipconf(){
echo ""
echo "jjjjjjjjjjjjjjjjjjjj=list=jjjjjjjjjjjjjjjjjjjj"
echo ""
echo "Ntp Server IP:$ntpip"
echo "Control Server IP:$controlip"
echo "Compute Server IP:$computeip"
echo "Metadata IP:$metadataip"
echo "Fixed_range:$fixed_range"
echo ""
echo "jjjjjjjjjjjjjjjjjjjj=list=jjjjjjjjjjjjjjjjjjjj"
echo ""
read -p "If the error,Please Ctrl+c end! If the OK! Press any key to start... "
}
...
##################################################wjj##################################################
#compute client: iscsi bridge and network
computeibn(){
...
#config test images nova-volume & only test!!!
...
sed -i '$i\losetup -f /srv/nova-volumes.img' /etc/rc.local
fi
...
##################################################wjj##################################################
novacompute(){
apt-get install -y nova-compute nova-network nova-vncproxy
RETVAL=$?
check $RETVAL "Nova compute network!!"
...
###### DATABASE
sql_connection=mysql://root:root@$controlip/nova
...
###### RABBITMQ
rabbit_host=$controlip

###### GLANCE
image_service=nova.image.glance.GlanceImageService
glance_api_servers=$controlip:9292
...
RETVAL=$?
check $RETVAL "Nova CONFIG!!"

echo 'for i in nova-compute nova-network nova-vncproxy; do /etc/init.d/$i restart;done' >/bin/allr.sh
chmod +x /bin/allr.sh
/bin/allr.sh
RETVAL=$?
check $RETVAL "Nova Restart!!"
touch /etc/nova/I.lock
fi
}
...
echo "a) OpenStack Essex All in one!!"
echo "c) Openstack Essex Compute node!"
read -p "#?" val
case "$val" in
    a)
    allinone
    ...
    ;;
    *)
    echo -e "\033[40;31mSelect Error!!\033[0m"
esac