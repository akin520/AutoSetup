...
download() {
	...
	echo "Download soft..."
	if [ ! -s nginx-1.1.17.tar.gz ]; then
	wget http://www.nginx.org/download/nginx-1.1.17.tar.gz
	fi
...
php() {
	...
	cd php-5.3.10
	./configure --prefix=/usr/local/php5310 --with-config-file-path=/usr/local/php5310/etc --with-mysql=/usr/local/percona --with-mysqli=/usr/local/percona/bin/mysql_config --with-fpm-user=webadm --with-fpm-group=webadm --with-iconv-dir=/usr/local --with-freetype-dir --with-jpeg-dir --with-png-dir --with-zlib --with-libxml-dir=/usr --enable-xml --disable-rpath --enable-bcmath --enable-shmop --enable-sysvsem --enable-inline-optimization --with-curl --with-curlwrappers --enable-mbregex --enable-fpm --enable-mbstring --with-mcrypt --with-gd --enable-gd-native-ttf --with-openssl --with-mhash --enable-pcntl --enable-sockets --with-ldap --with-ldap-sasl --with-xmlrpc --enable-zip --enable-soap	
